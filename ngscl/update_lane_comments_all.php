...
if (isset($_POST['process']))
{
  if ($_POST['process'] == 1)
  {
    if (isset($_POST['submit_return_to_details']))
    {
      ...
      unset($_POST['submit_save_comments']);
      header("location: manage_lane_all_samples.php?&sticky_lane=".
             $_SESSION['lane_number']);
      exit;
    } elseif (isset($_POST['clear_all_remove'])) {
      unset($_POST['run_lane_sample_uid']);
...
// Get the run number and name.
  ...
  echo '<input type="submit" name="submit_exit" value="Quit" ',
       ...
       'title="Return to Manage Lane All Samples page without saving." ',
       'class="buttontext" />';
  ...
  echo '<tbody>';
  // ****
  // Build a where condition to set the status, PI, and project
  // so that the all sample query on the update lane comments page
  // is the same as on the manage lage page.
  // ****
  // Status condition.
  $status_value = (isset ($_SESSION['choose_sample_status']) ?
   $_SESSION['choose_sample_status'] : 0);
