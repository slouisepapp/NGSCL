...
if ($use_sample_bonus_columns)
{
  ...
  $example_standard_barcode = 'example_sample_plus_standard_barcode.png';
} else {
  $sample_key_array = array(0 => 'sample_name',
                            1 => 'sample_description',
                            2 => 'status',
                            ...
                            8 => 'comments');
  $sample_header_array = $sample_key_array;
  $example_standard_barcode = 'example_sample_standard_barcode.png';
}  // if ($use_sample_bonus_columns)
      ...
      if (isset($_FILES['userfile']))
      {
            ...
            ($_FILES['userfile']['error'] == UPLOAD_ERR_OK))
        {
          ...
          if (count($array_error) < 1)
          {
            ...
            // Add the barcode error array to array_error.
            if (count($barcode_array_error) > 0)
            {
              $array_error = error_array_merge (
                                $array_error, $barcode_array_error);
            }  // if (count($barcode_array_error) > 0)
            if (count($array_error) < 1)
            {
