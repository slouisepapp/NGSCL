    ...
    $_SESSION['run_number'] < 0)
{
  ...
  if (!$result)
  {
    ...
    array_push ($array_error,
     '* A '.$run_type.' run number '.$run_number.
...
if (isset($_SESSION['run_name']) &&
    trimmed_string_not_empty ($_SESSION['run_name']))
{
  $run_name = ddl_ready ($_SESSION['run_name']); 
}  // if(isset($_SESSION['run_name']) &&...
...
{
   array_push($array_error,
    "* You must enter a positive integer for Indexing Read Length");
} else {
...
if (isset($_SESSION['hi_seq_slot']) &&
    trimmed_string_not_empty ($_SESSION['hi_seq_slot']))
{
...
if (isset($_SESSION['cluster_gen_start_date']) &&
    trimmed_string_not_empty ($_SESSION['cluster_gen_start_date']))
{
...
if (isset($_SESSION['sequencing_start_date']) &&
    trimmed_string_not_empty ($_SESSION['sequencing_start_date']))
{
    ...
    trimmed_string_not_empty ($_SESSION['run_number']))
{
  ...
  $run_uid = pg_fetch_result ($result, 0, 0);
  $_SESSION['run_uid'] = $run_uid;
  // Set the date strings for the insert statement.
  ...
  $date_fields_string = $cluster_gen_start_date_string . "," .
                        $sequencing_start_date_string . ",";
  $result = pg_query ($dbconn, "
    INSERT INTO run
      (run_uid, run_number, run_name, ref_run_type_uid, read_type,
       read_1_length, read_2_length, read_length_indexing, hi_seq_slot,
       cluster_gen_start_date, sequencing_start_date,
       truseq_cluster_gen_kit, flow_cell_hs_id, sequencing_kits,
       comments)
    VALUES
      ($run_uid, $run_number, '$run_name', $ref_run_type_uid, '$read_type',
       $read_1_length, $read_2_length, $read_length_indexing, $hi_seq_slot," .
