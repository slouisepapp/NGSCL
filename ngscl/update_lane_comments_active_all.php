...
<?php
  echo '<h1 style="text-align:center"><span class="titletext">',
       'Update Run Lanes Comments - ',$app_name,'</span></h1>';
  echo '<!-- end #header --></div>';
  ...
  echo '<input type="submit" name="submit_exit" value="Quit" ',
      ...
      'title="Return to Manage All Active Samples page without saving." ',
      'class="buttontext" />';
  ...
  echo '<tbody>';
  // ****
  // Build a where condition to set the PI
  // so that the active sample query on the update lane comments page
  // is the same as on the manage lage page.
  // ****
  $primary_investigator_uid_value = $_SESSION['choose_pi'];
